function [ lambda_fix ] = entropy_fix_x(V, lambda, alpha, g, ...
        h_L, hu_L, hv_L, h_R, hu_R, hv_R)
%ENTROPY_FIX_X Compute a modified set of eigenvalues according to the entropy
...
for k = 1:3
    ...
    u_L_int = hu_L_int / h_L_int;
    u_R_int = hu_R_int / h_R_int;

    % Compute characteristic speeds (eigenvalues of the Jacobian)
    lambda_L_int(1) = u_L_int;
    lambda_L_int(2) = u_L_int + c_L_int;
    lambda_L_int(3) = u_L_int - c_L_int;

    lambda_R_int(1) = u_R_int;
    lambda_R_int(2) = u_R_int + c_R_int;
    lambda_R_int(3) = u_R_int - c_R_int;
